...
def render_market_overview(indices_data):
    """
    Hiển thị thanh chỉ số ngang (Ticker Tape)
    """
    ...
    for i, data in enumerate(indices_data):
        with cols[i]:
            ...
            if status == "LIVE":
                ...
                # Format giá: Có dấu phẩy ngăn cách hàng nghìn
...
# ==============================================================================
# 2. BỘ THẺ PHÂN TÍCH KÉP (DUAL ANALYSIS CARDS) - THEO YÊU CẦU
# ==============================================================================
def render_analysis_section(tech, fund):
    """
    Vẽ 2 thẻ bài: Kỹ thuật (Trái) & Cơ bản (Phải)
    """
    c1, c2 = st.columns(2)
    
    # --- THẺ 1: GÓC NHÌN KỸ THUẬT ---
    with c1:
        st.markdown(f"""
        <div style="
            background-color: #1f2937; border-radius: 12px; padding: 20px; 
            border: 1px solid #374151; border-left: 5px solid {tech['color']};
        ...
        ">
            ...
            <div style="
                ...
                font-family: 'Rajdhani'; font-size: 2.5rem; font-weight: 800;
                margin: 15px 0; box-shadow: 0 0 20px {tech['color']}60;
            ">
                {tech['score']}
            </div>
            ...
            <div style="display:flex; justify-content:space-between; font-family:'Rajdhani';">
                <div>
                    <div style="color:#9ca3af; font-size:0.8rem;">ENTRY</div>
                    <div style="color:white; font-weight:700; font-size:1.1rem;">{tech['entry']:,.0f}</div>
                </div>
                <div>
                    <div style="color:#ef4444; font-size:0.8rem;">STOPLOSS</div>
                    <div style="color:#ef4444; font-weight:700; font-size:1.1rem;">{tech['stop']:,.0f}</div>
                </div>
                <div>
                    <div style="color:#10b981; font-size:0.8rem;">TARGET</div>
                    <div style="color:#10b981; font-weight:700; font-size:1.1rem;">{tech['target']:,.0f}</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Expander chi tiết
        with st.expander("🔍 Chi tiết Kỹ Thuật (Lý do)", expanded=False):
            for p in tech['pros']: st.success(f"✅ {p}")
            for c in tech['cons']: st.error(f"⚠️ {c}")

    # --- THẺ 2: SỨC KHỎE DOANH NGHIỆP ---
    with c2:
        st.markdown(f"""
        <div style="
            background-color: #1f2937; border-radius: 12px; padding: 20px; 
            border: 1px solid #374151; border-left: 5px solid {fund['color']};
            text-align: center; box-shadow: 0 4px 10px rgba(0,0,0,0.3);
        ...
        ">
            <div>
                ...
                <div style="margin: 20px 0;">
                    <h2 style="color:{fund['color']}; margin:0; font-size:2rem; font-weight:900; line-height:1.2;">
                        {fund['health']}
                    </h2>
                </div>
            </div>
            ...
            <div>
                ...
                <div style="color: #cbd5e1; font-weight: 500;">
                    Vốn hóa thị trường: <span style="color:white; font-weight:700;">{fund['market_cap']/1e9:,.0f} Tỷ</span>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Expander chi tiết
        with st.expander("🔍 Chi tiết Cơ Bản (BCTC)", expanded=True):
            for d in fund['details']:
                if "cao" in d or "Thấp" in d or "giảm" in d:
                    st.warning(f"⚠️ {d}")
                else:
                    st.success(f"✅ {d}")

# ==============================================================================
# 3. BIỂU ĐỒ TƯƠNG TÁC (INTERACTIVE CHART)
# ==============================================================================
def render_interactive_chart(df, symbol):
    """
    Vẽ biểu đồ Nến + Volume + Ichimoku
    """
    if df.empty:
        st.warning("Chưa có dữ liệu biểu đồ.")
        return

    # Tính Ichimoku để vẽ mây (nếu chưa có)
    try:
    ...
    # Tạo Subplot: Giá (Trên) - Volume (Dưới)
    fig = make_subplots(
        rows=2, cols=1, 
        shared_xaxes=True, 
    ...
    # 3. Volume Bar (Màu theo nến tăng/giảm)
    ...
    fig.update_layout(
        ...
        height=600,
        xaxis_rangeslider_visible=False, # Tắt cái slider mặc định xấu xí
    ...
    # Bật thanh trượt range slider ở trục X
